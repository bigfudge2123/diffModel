...
class Trainer(torch.nn.Module):
    def __init__(self, glctx, geometry, lgt, mat, optimize_geometry, optimize_light, image_loss_fn, FLAGS):
        ...
        self.handles = [] # 用于存放钩子，用完之后 .remove() 删除钩子   
    ...
    #     def module_forward_func(module, ten_in, ten_out):
    #         print(ten_in, ten_out)   # 前向到这层 layer 时打印它的输入输出
    #     # self.handles.append(module.register_forward_hook( module_forward_func ))

    #     def module_backward_func(module, grad_out, grad_in):
    #         print(grad_in, grad_out)
    #         return grad_out          # 这里可以修改 grad_out
    #     self.handles.append(module.register_full_backward_hook( module_backward_func ))
...
def optimize_mesh(
    ...
    print("---打印全部参数的名字---")
    print("state_dict:", list(trainer_noddp.state_dict())) 
        ...
        if FLAGS.local_rank == 0:
            ...
            if display_image or save_image:
                ...
                if display_image:
                    ...
                    # util.save_image(FLAGS.out_dir + '/' + ('%06d.jpg' % (img_cnt)), np_result_image)
                if save_image:
    ...
    ax = fig.gca()
    ax.plot(img_loss_vec, label="image loss")
...
if __name__ == "__main__":
    ...
    if FLAGS.base_mesh is None:
        ...
        geometry, mat = optimize_mesh(glctx, geometry, mat, lgt, dataset_train, dataset_validate, 
                        FLAGS, pass_idx=0, pass_name="dmtet_pass1", optimize_light=FLAGS.learn_light, optimize_geometry=True)
